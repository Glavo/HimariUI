...
/// bindings update stable properties, while explicit `ForEach` and `Show` anchors are the only
/// operations permitted to change topology.
@NotNullByDefault
final class OneShotSettingsChatApplication extends OneShotFixtureSession {
    ...
    /// The message collection controller, or `null` before mount.
    private @Nullable OneShotRuntime.KeyedItems<String, OneShotRuntime.LocalInt> reactions;

    /// The retained advanced-panel controller, or `null` before mount.
    private @Nullable OneShotRuntime.Show<OneShotRuntime.LocalInt> advanced;
    ...
    OneShotSettingsChatApplication(ComparisonEnvironment environment, ComparisonProbe probe) {
    ...
    protected @Unmodifiable Map<String, String> values() {
        @Nullable OneShotRuntime.LocalInt advancedLocal = advancedController().value();
        return valuesOf(
    ...
    ///
    /// @param root the root owner
    private void initializeSettings(OneShotRuntime.Owner root) {
    ...
    ///
    /// @param root the root owner
    private void initializeChat(OneShotRuntime.Owner root) {
        root.component("chat-owner", chat -> {
            ...
            reactions = chat.forEach(
                    "message-rows",
                    visibleKeys,
                    visibleKeys::get,
                    (message, key) -> {
                        OneShotRuntime.LocalInt local = message.localInt(0);
    ...
    ///
    /// @param root the root owner
    private void initializeAdvancedSettings(OneShotRuntime.Owner root) {
        advanced = root.show(
                "advanced-settings",
                advancedVisible,
                advancedVisible::get,
                ...
                panel -> {
                    OneShotRuntime.LocalInt local = panel.localInt(0);
    ...
    private OneShotRuntime.KeyedItems<String, OneShotRuntime.LocalInt> reactionController() {
        if (reactions == null) {
    ...
    private OneShotRuntime.LocalInt reaction(String key) {
        @Nullable OneShotRuntime.LocalInt local = reactionController().value(key);
        if (local == null) {
    ...
    ///
    /// @return the controller
    private OneShotRuntime.Show<OneShotRuntime.LocalInt> advancedController() {
        if (advanced == null) {
    ...
    private OneShotRuntime.LocalInt advanced() {
        @Nullable OneShotRuntime.LocalInt local = advancedController().value();
        if (local == null || !advancedController().visible()) {
