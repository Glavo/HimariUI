...
/// Supplies the six ordinary-Java micro applications for the one-shot candidate.
@NotNullByDefault
final class OneShotMicroApplications {
    /// Prevents construction.
    private OneShotMicroApplications() {
    }
    ...
    @NotNullByDefault
    private static final class CounterApplication extends OneShotFixtureSession {
    ...
    @NotNullByDefault
    private static final class DiamondApplication extends OneShotFixtureSession {
        /// The diamond source.
    ...
    @NotNullByDefault
    private static final class ConditionalApplication extends OneShotFixtureSession {
        ...
        /// The retained conditional controller, or `null` before mount.
        private @Nullable OneShotRuntime.Show<OneShotRuntime.LocalInt> retained;

        /// The disposable conditional controller, or `null` before mount.
        ...
        /// Mounts two explicit `Show` anchors with different hidden lifetimes.
        ///
        /// @param root the root owner
        @Override
        protected void initialize(OneShotRuntime.Owner root) {
            ...
            retained = root.show(
                    ...
                    branch -> {
                        OneShotRuntime.LocalInt local = branch.localInt(0);
        ...
        protected boolean handle(FixtureCommand command) {
            return switch (command.operation()) {
                ...
                case "increment-retained" -> {
                    requireLocal(retained().value(), "retained").increment();
                    yield false;
                }
                case "show-disposed" -> {
                ...
                case "increment-disposed" -> {
                    requireLocal(disposed().value(), "disposed").increment();
                    yield false;
                }
                default -> throw unknown(command);
            };
        ...
        @Override
        protected @Unmodifiable Map<String, String> values() {
            @Nullable OneShotRuntime.LocalInt retainedLocal = retained().value();
            @Nullable OneShotRuntime.LocalInt disposedLocal = disposed().value();
        ...
        /// @return the local
        private static OneShotRuntime.LocalInt requireLocal(
                @Nullable OneShotRuntime.LocalInt local,
                String name
        ) {
            if (local == null) {
    ...
    /// Implements changing child input with a stable one-shot component owner.
    @NotNullByDefault
    private static final class ChangingInputApplication extends OneShotFixtureSession {
        /// The parent-owned child input.
        private final MutableState<String> input;

        /// The child-local cell, or `null` before mount.
        private @Nullable OneShotRuntime.LocalInt childLocal;
        ...
        ///
        /// @return the cell
        private OneShotRuntime.LocalInt local() {
            if (childLocal == null) {
    ...
    @NotNullByDefault
    private static final class KeyedListApplication extends OneShotFixtureSession {
        ...
        /// Mounts one explicit `ForEach` anchor keyed by application item identity.
        ///
        /// @param root the root owner
        @Override
        protected void initialize(OneShotRuntime.Owner root) {
    ...
    @NotNullByDefault
    private static final class PhaseImpactApplication extends OneShotFixtureSession {
