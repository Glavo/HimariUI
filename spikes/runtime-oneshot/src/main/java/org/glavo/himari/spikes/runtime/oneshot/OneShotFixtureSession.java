...
/// Provides command, observation, transaction, diagnostic, and cleanup plumbing for one-shot apps.
@NotNullByDefault
abstract class OneShotFixtureSession implements RuntimeFixtureSession {
    ...
    /// The candidate-specific one-shot owner runtime.
    private final OneShotRuntime runtime;

    /// Whether the one-shot root has mounted.
    ...
    protected OneShotFixtureSession(ComparisonEnvironment environment, ComparisonProbe probe) {
        Objects.requireNonNull(environment, "environment");
        this.domain = environment.stateDomain();
        this.probe = Objects.requireNonNull(probe, "probe");
        this.runtime = new OneShotRuntime(probe);
    }

    /// Executes one command and flushes changed bindings or explicit structural controllers.
    ///
    ...
    public final void execute(FixtureCommand command) {
        ...
        } catch (OneShotRuntime.OneShotMutationException failure) {
            onMutationFailure(failure);
        }
    }

    /// Returns application values and the committed owner-tree nodes.
    ///
    ...
    /// Enters benchmark mode and suppresses semantically irrelevant lifecycle events.
    @Override
    ...
    /// Returns live one-shot-runtime resources, or the clean sentinel after closure.
    ///
    ...
    /// Releases the owner tree and application-owned state resources.
    ///
    ...
    /// Initializes the complete persistent application owner tree exactly once.
    ///
    /// @param root the root owner
    protected abstract void initialize(OneShotRuntime.Owner root);

    /// Applies one non-mount application command.
    ///
    /// @param command the command
    /// @return whether changed reactive sources must be flushed
    protected abstract boolean handle(FixtureCommand command);

    /// Returns current application-owned observable values.
    ///
    /// @return the immutable value map
    protected abstract @Unmodifiable Map<String, String> values();

    /// Handles a contained structural mutation failure after preserving the committed owner tree.
    ///
    ...
    /// Releases application resources not owned by the one-shot tree.
    protected void closeApplicationResources() {
    }

    /// Runs source writes as one state-domain publication.
    ///
    ...
    /// Emits a quality-four structured trace and records its stable diagnostic code.
    ///
    ...
    protected final void traceDiagnostic(String code, String ownerPath, String recoveryAction) {
        ...
        probe.trace(new DiagnosticTrace(
                code,
                "One-shot application callback failed inside its declared boundary",
                "oneshot-app",
                ownerPath,
                null,
                recoveryAction
