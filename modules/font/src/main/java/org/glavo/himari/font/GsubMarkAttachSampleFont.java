...
/// Generates a face whose GSUB `calt` and `rlig` lookups use `MarkAttachmentType` 1.
///
/// GDEF marks `B` with attach class 2 and `D` with attach class 1. `A` plus `C` therefore skips
/// `B` and keeps `D`.
@NotNullByDefault
public final class GsubMarkAttachSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Ligature and substitute advance.
    ...
    /// Substitute and ligature glyph of `X`.
    ...
    private GsubMarkAttachSampleFont() {
    }
    ...
    /// Writes type-5 `calt`, nested type-1, and type-4 `rlig` with `MarkAttachmentType` 1.
    private static byte[] gsub() {
        ...
        lookup2.putShort((short) (ATTACH_TYPE << 8));
    ...
    private static byte[] name() {
        byte[] family = "HimariGsubAttach".getBytes(StandardCharsets.US_ASCII);
