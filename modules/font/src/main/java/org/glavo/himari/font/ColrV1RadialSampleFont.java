...
/// Generates a TrueType face whose `A` is a COLR v1 glyph filled by `PaintRadialGradient`.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. The paint graph is
/// `PaintGlyph` wrapping a one-stop `PaintRadialGradient`.
@NotNullByDefault
public final class ColrV1RadialSampleFont {
    /// Units per em.
    ...
    private ColrV1RadialSampleFont() {
    }
    ...
    /// Writes COLR v1 with `PaintGlyph` filled by a one-stop radial gradient.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrRadial".getBytes(StandardCharsets.US_ASCII);
