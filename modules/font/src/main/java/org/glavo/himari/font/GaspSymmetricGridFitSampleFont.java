...
/// Generates a face whose `gasp` table requests symmetric grid fitting at every ppem.
@NotNullByDefault
public final class GaspSymmetricGridFitSampleFont {
    ...
    private GaspSymmetricGridFitSampleFont() {
    }
    ...
    private static byte[] gasp() {
        ...
        buffer.putShort((short) 1);
        buffer.putShort((short) 0xFFFF);
        buffer.putShort((short) (GaspTable.SYMMETRIC_GRIDFIT | GaspTable.DOGRAY));
        return buffer.array();
    }

    /// Writes a triangle whose x extents are `1` to `7` so 12 ppem is off the pixel grid.
    private static byte[] glyf() {
        ...
        output.write(0x01);
        output.write(0x01);
        BitmapSfntFont.writeShorts(output, (short) 1, (short) 6, (short) -6);
    ...
    private static byte[] name() {
        byte[] family = "HimariGaspSymGrid".getBytes(StandardCharsets.US_ASCII);
