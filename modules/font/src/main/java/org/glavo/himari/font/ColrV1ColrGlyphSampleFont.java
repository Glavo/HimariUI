...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintColrGlyph` of a solid glyph.
///
/// Glyph 2 is the empty public base. Glyph 3 is the painted rectangle. Glyph 4 is the
/// referenced paint graph `PaintGlyph`/`PaintSolid`.
@NotNullByDefault
public final class ColrV1ColrGlyphSampleFont {
    /// Units per em.
    ...
    /// Nested paint-graph glyph referenced by `PaintColrGlyph`.
    public static final int GLYPH_NESTED = 4;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 5;

    /// Prevents instantiation.
    private ColrV1ColrGlyphSampleFont() {
    }

    /// Builds the COLR v1 `PaintColrGlyph` sample font.
    ///
    ...
    /// Builds the COLR v1 `PaintColrGlyph` sample font image.
    ///
    ...
    /// Writes long `loca` offsets. Glyphs 0–2 and 4 are empty.
    ...
    /// Writes COLR v1 with `PaintColrGlyph` referencing a nested solid glyph.
    private static byte[] colr() {
        ...
        int outerRel = buffer.position();
        putOffset24(buffer, 0);
        buffer.putShort((short) GLYPH_NESTED);
        int nestedRel = buffer.position();
        putOffset24(buffer, 0);
        int colrGlyph = buffer.position();
        buffer.put((byte) 11);
        buffer.putShort((short) GLYPH_NESTED);
        putOffset24At(buffer, outerRel, colrGlyph - baseList);
        int glyphPaint = buffer.position();
        writePaintGlyph(buffer, GLYPH_BACK, 0);
        putOffset24At(buffer, nestedRel, glyphPaint - baseList);
        return slice(buffer);
    }
    ...
    private static byte[] name() {
        byte[] family = "HimariColrGlyph".getBytes(StandardCharsets.US_ASCII);
