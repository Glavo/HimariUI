...
/// only after backtrack `NMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainUndecaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `NMLKJIHGFEDABC`.
    ...
    private GposChainUndecaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`N` range.
    ...
    /// Writes ChainContextPos format 1 for `NMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(56).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain11Back".getBytes(StandardCharsets.US_ASCII);
