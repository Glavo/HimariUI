...
/// Generates a TrueType face whose `A` is a COLR v1 glyph filled by `PaintSweepGradient`.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. The paint graph is
/// `PaintGlyph` wrapping a one-stop `PaintSweepGradient`.
@NotNullByDefault
public final class ColrV1SweepSampleFont {
    /// Units per em.
    ...
    /// Writes COLR v1 with `PaintGlyph` filled by a one-stop sweep gradient.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrSweep".getBytes(StandardCharsets.US_ASCII);
