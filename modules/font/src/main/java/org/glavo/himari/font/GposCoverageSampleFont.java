...
/// Generates a face whose GPOS type-7/8 format-3 coverage rules adjust `A`.
///
/// Lookup 0 is ContextPos format 3 for `AB`. Lookup 1 is the nested type-1. Lookup 2 is
/// ChainContextPos format 3 for `AB` with lookahead `C`.
@NotNullByDefault
public final class GposCoverageSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Format-3 context X-advance applied to `A` before `B`.
    public static final int CONTEXT_DELTA = 5;

    /// Format-3 chain X-advance applied to `A` before `B` with lookahead `C`.
    public static final int CHAIN_DELTA = 7;
    ...
    private GposCoverageSampleFont() {
    }
    ...
    /// Writes format-3 context, nested type-1, and format-3 chain lookups.
    private static byte[] gpos() {
        byte[] contextNested = singlePos(CONTEXT_DELTA);
        ByteBuffer lookup1 = ByteBuffer.allocate(8 + contextNested.length).order(ByteOrder.BIG_ENDIAN);
        lookup1.putShort((short) 1);
        lookup1.putShort((short) 0);
        lookup1.putShort((short) 1);
        lookup1.putShort((short) 8);
        lookup1.put(contextNested);
        byte[] chainNested = singlePos(CHAIN_DELTA);
        ByteBuffer lookup3 = ByteBuffer.allocate(8 + chainNested.length).order(ByteOrder.BIG_ENDIAN);
        lookup3.putShort((short) 1);
        lookup3.putShort((short) 0);
        lookup3.putShort((short) 1);
        lookup3.putShort((short) 8);
        lookup3.put(chainNested);
        byte[] context = contextPosFormat3();
        ...
        lookup0.put(context);
        byte[] chain = chainPosFormat3();
        ByteBuffer lookup2 = ByteBuffer.allocate(8 + chain.length).order(ByteOrder.BIG_ENDIAN);
        ...
        ByteBuffer feature = ByteBuffer.allocate(8).order(ByteOrder.BIG_ENDIAN);
        feature.putShort((short) 0);
        feature.putShort((short) 2);
        feature.putShort((short) 0);
        feature.putShort((short) 2);
    ...
    /// Writes ContextPos format 3 for `AB` using nested lookup 1.
    private static byte[] contextPosFormat3() {
    ...
    /// Writes ChainContextPos format 3 for `ABC` using nested lookup 3.
    private static byte[] chainPosFormat3() {
    ...
    private static byte[] singlePos(int delta) {
    ...
    private static byte[] name() {
        byte[] family = "HimariGposCov".getBytes(StandardCharsets.US_ASCII);
