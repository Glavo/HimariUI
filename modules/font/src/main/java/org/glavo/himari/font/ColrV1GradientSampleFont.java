...
/// Generates a TrueType face whose `A` is a COLR v1 glyph filled by `PaintLinearGradient`.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. The paint graph is
/// `PaintGlyph` wrapping a one-stop `PaintLinearGradient`.
@NotNullByDefault
public final class ColrV1GradientSampleFont {
    /// Units per em.
    ...
    private ColrV1GradientSampleFont() {
    }

    /// Builds the COLR v1 gradient sample font.
    ///
    ...
    /// Writes COLR v1 with `PaintGlyph` filled by a one-stop linear gradient.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrGradient".getBytes(StandardCharsets.US_ASCII);
