...
@NotNullByDefault
public final class GposChainOctodecaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `UTSRQPONMLKJIHGFEDABC`.
    ...
    private static final int GLYPH_COUNT = 23;

    /// Prevents instantiation.
    private GposChainOctodecaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`U` range.
    ...
    /// Writes ChainContextPos format 1 for `UTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    ...
    private static byte[] name() {
        byte[] family = "HimariChain18Back".getBytes(StandardCharsets.US_ASCII);
