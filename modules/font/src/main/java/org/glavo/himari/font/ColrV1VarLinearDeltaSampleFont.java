...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintVarLinearGradient` with a store.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. The default instance uses
/// palette 0 (red). Peak `wght` applies an ItemVariationStore delta of `1` so the first
/// `VarColorLine` stop becomes palette 1 (blue).
@NotNullByDefault
public final class ColrV1VarLinearDeltaSampleFont {
    /// `wght` tag.
    ...
    private ColrV1VarLinearDeltaSampleFont() {
    }

    /// Builds the COLR v1 `PaintVarLinearGradient` variation sample font.
    ///
    ...
    /// Builds the COLR v1 `PaintVarLinearGradient` variation sample font image.
    ///
    ...
    /// Writes COLR v1 with `PaintGlyph` filled by `PaintVarLinearGradient` and a one-region store.
    private static byte[] colr() {
        ...
        putOffset24At(buffer, paintRelPos, glyphPaint - baseList);
        int varLinear = buffer.position();
        buffer.put((byte) 5);
        int linePos = buffer.position();
        putOffset24(buffer, 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        ...
        buffer.putShort((short) 8);
        buffer.putInt(-1);
        putOffset24At(buffer, childPos, varLinear - glyphPaint);
        int colorLine = buffer.position();
        buffer.put((byte) 0);
        buffer.putShort((short) 1);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        ...
        buffer.putInt(0);
        putOffset24At(buffer, linePos, colorLine - varLinear);
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarLinDelta".getBytes(StandardCharsets.US_ASCII);
