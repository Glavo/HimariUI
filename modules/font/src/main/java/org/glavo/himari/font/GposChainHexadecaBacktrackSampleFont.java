...
/// only after backtrack `SRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainHexadecaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `SRQPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `S`.
    public static final int GLYPH_S = 20;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 21;

    /// Prevents instantiation.
    private GposChainHexadecaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`S` range.
    ...
    /// Writes ChainContextPos format 1 for `SRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(66).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain16Back".getBytes(StandardCharsets.US_ASCII);
