...
/// only after backtrack `RQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainPentadecaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `RQPONMLKJIHGFEDABC`.
    ...
    private GposChainPentadecaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`R` range.
    ...
    /// Writes ChainContextPos format 1 for `RQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    ...
    private static byte[] name() {
        byte[] family = "HimariChain15Back".getBytes(StandardCharsets.US_ASCII);
