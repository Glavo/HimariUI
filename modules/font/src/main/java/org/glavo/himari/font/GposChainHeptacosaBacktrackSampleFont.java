...
/// only after backtrack `4321ZYXWVUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainHeptacosaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `4321ZYXWVUTSRQPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `4`.
    public static final int GLYPH_FOUR = 31;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 32;

    /// Prevents instantiation.
    private GposChainHeptacosaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the digits `1`–`4` and the contiguous `A`–`Z` range.
    ...
    /// Writes twenty-seven rectangles.
    ...
    /// Writes ChainContextPos format 1 for `4321ZYXWVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(88).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 82);
        buffer.putShort((short) 1);
        buffer.putShort((short) 8);
        buffer.putShort((short) 1);
        buffer.putShort((short) 4);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain27Back".getBytes(StandardCharsets.US_ASCII);
