...
/// only after backtrack `ONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainDodecaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `ONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `O`.
    public static final int GLYPH_O = 16;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 17;

    /// Prevents instantiation.
    private GposChainDodecaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`O` range.
    ...
    /// Writes ChainContextPos format 1 for `ONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(58).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 52);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain12Back".getBytes(StandardCharsets.US_ASCII);
