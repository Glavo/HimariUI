...
/// Generates a face whose GSUB `calt` type-6 format-3 coverage rule turns `A` into `Y` before `BC`.
@NotNullByDefault
public final class GsubChainCoverageSampleFont {
    ...
    /// Writes a format-4 cmap for space, `A`, `B`, `C`, and `Y`.
    ...
    /// Writes ChainContextSubst format 3 for `ABC`.
    private static byte[] chainSubst() {
        ByteBuffer buffer = ByteBuffer.allocate(38).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 3);
        buffer.putShort((short) 0);
        buffer.putShort((short) 2);
    ...
    private static byte[] hmtx() {
        ...
        int[] advances = {0, 3, ADVANCE_LETTER, ADVANCE_LETTER, ADVANCE_LETTER, ADVANCE_Y};
    ...
    private static byte[] name() {
        byte[] family = "HimariChain3".getBytes(StandardCharsets.US_ASCII);
