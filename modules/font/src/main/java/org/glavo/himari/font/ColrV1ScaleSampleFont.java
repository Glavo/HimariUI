...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintScale` of one solid glyph.
///
/// Glyph 2 is the empty base. Glyph 3 is the red rectangle. The paint graph is
/// `PaintScale` wrapping `PaintGlyph`/`PaintSolid`.
@NotNullByDefault
public final class ColrV1ScaleSampleFont {
    /// Units per em.
    ...
    private ColrV1ScaleSampleFont() {
    }

    /// Builds the COLR v1 scale sample font.
    ///
    ...
    /// Builds the COLR v1 scale sample font image.
    ///
    ...
    /// Writes COLR v1 with a `PaintScale` wrapping one solid glyph.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrScale".getBytes(StandardCharsets.US_ASCII);
