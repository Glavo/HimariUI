...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintVarScale` with a store.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. Default `scaleX` is `0x4000`
/// (`1.0` as `F2DOT14`). Peak `wght` applies an ItemVariationStore delta of `3` so
/// [`ColorLayer#scaleY()`] is `0x4003`.
@NotNullByDefault
public final class ColrV1VarScaleYDeltaSampleFont {
    /// `wght` tag.
    ...
    /// Default `scaleY` before variation, as `F2DOT14` `1.0`.
    public static final int BASE_SCALE_Y = 0x4000;

    /// ItemVariationStore delta applied to `scaleY` at peak `wght`.
    public static final int SCALE_Y_DELTA = 3;
    ...
    private ColrV1VarScaleYDeltaSampleFont() {
    }

    /// Builds the COLR v1 `PaintVarScale` variation sample font.
    ///
    ...
    /// Builds the COLR v1 `PaintVarScale` variation sample font image.
    ///
    ...
    /// Writes COLR v1 with `PaintVarScale` wrapping `PaintGlyph`/`PaintSolid` and a store.
    private static byte[] colr() {
        ...
        putOffset24(buffer, 0);
        buffer.putShort((short) BASE_SCALE_Y);
        buffer.putShort((short) BASE_SCALE_Y);
        buffer.putInt(0);
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarSy".getBytes(StandardCharsets.US_ASCII);
