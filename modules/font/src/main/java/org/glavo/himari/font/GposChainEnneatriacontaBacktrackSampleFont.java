...
@NotNullByDefault
public final class GposChainEnneatriacontaBacktrackSampleFont {
    ...
    private static final int GLYPH_COUNT = 44;

    /// Prevents instantiation.
    private GposChainEnneatriacontaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the digits `1`–`9` and `:`/`;`/`<`/`=`/`>`/`?`/`@` and the contiguous `A`–`Z` range.
    ...
    private static byte[] name() {
        byte[] family = "HimariChain39Back".getBytes(StandardCharsets.US_ASCII);
