...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintVarScaleAroundCenter` with a store.
///
...
@NotNullByDefault
public final class ColrV1VarScaleAroundCenterDeltaSampleFont {
    /// `wght` tag.
    ...
    private ColrV1VarScaleAroundCenterDeltaSampleFont() {
    }
    ...
    private static byte[] colr() {
        ByteBuffer buffer = ByteBuffer.allocate(192).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 0);
        buffer.putInt(0);
        buffer.putInt(0);
