...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintVarRotate` with a store.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. Default angle is `2` as
/// `F2DOT14`. Peak `wght` applies an ItemVariationStore delta of `3` so [`ColorLayer#rotate()`]
/// is `5`.
@NotNullByDefault
public final class ColrV1VarRotateCenterYDeltaSampleFont {
    /// `wght` tag.
    ...
    /// Default around-center Y before variation.
    public static final int BASE_CENTER_Y = 2;

    /// ItemVariationStore delta applied to center Y at peak `wght`.
    public static final int CENTER_DELTA = 3;
    ...
    private ColrV1VarRotateCenterYDeltaSampleFont() {
    }

    /// Builds the COLR v1 `PaintVarRotate` variation sample font.
    ///
    ...
    /// Builds the COLR v1 `PaintVarRotate` variation sample font image.
    ///
    ...
    /// Writes COLR v1 with `PaintVarRotate` wrapping `PaintGlyph`/`PaintSolid` and a store.
    private static byte[] colr() {
        ...
        int rotate = buffer.position();
        buffer.put((byte) 27);
        int childPos = buffer.position();
        putOffset24(buffer, 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) BASE_CENTER_Y);
        buffer.putInt(0);
        putOffset24At(buffer, paintRelPos, rotate - baseList);
        int glyphPaint = buffer.position();
        buffer.put((byte) 10);
        int fillPos = buffer.position();
        putOffset24(buffer, 0);
        buffer.putShort((short) GLYPH_BACK);
        putOffset24At(buffer, childPos, glyphPaint - rotate);
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarCy".getBytes(StandardCharsets.US_ASCII);
