...
@NotNullByDefault
public final class GsubChainDocosaBacktrackSampleFont {
    ...
    /// Substitute glyph mapped from `Z`.
    ...
    private GsubChainDocosaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space, the contiguous `A`–`Y` range, and `Z`.
    ...
    /// Writes ChainContextSubst format 1 for `YXWVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    ...
    /// Writes SingleSubst format 2 `A` to `Z`.
    ...
    private static byte[] hmtx() {
        ...
        for (int index = 0; index < GLYPH_COUNT; index++) {
            buffer.putShort((short) (index == GLYPH_Z ? ADVANCE_Y : index < 2 ? (index == 0 ? 0 : 3) : ADVANCE_LETTER));
            buffer.putShort((short) 0);
        }
        return buffer.array();
    }
    ...
    private static byte[] name() {
        byte[] family = "HimariGsubChain20Back".getBytes(StandardCharsets.US_ASCII);
