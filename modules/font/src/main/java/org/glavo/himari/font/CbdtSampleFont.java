...
/// Generates a TrueType face with one CBLC/CBDT format-1 strike for `A`.
///
/// The strike is 8 ppem and 8-bit. Glyph 1 is a 2×2 coverage square.
@NotNullByDefault
public final class CbdtSampleFont {
    /// Strike ppem.
    public static final int PPEM = 8;

    /// Expected coverage pixels.
    public static final byte @Unmodifiable [] PIXELS = {(byte) 255, (byte) 128, (byte) 128, (byte) 255};
    ...
    private CbdtSampleFont() {
    }

    /// Builds the CBDT sample font.
    ///
    ...
    /// Builds the CBDT sample font image.
    ///
    ...
    public static MemorySegment bytes() {
        ...
        tables.put("CBLC", cblc());
        tables.put("CBDT", cbdt());
    ...
    /// Writes 2×2 eight-bit coverage after the CBDT header.
    private static byte[] cbdt() {
    ...
    private static byte[] name() {
        byte[] family = "HimariCbdt".getBytes(StandardCharsets.US_ASCII);
