...
/// only after backtrack `7654321ZYXWVUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainTriacontaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `7654321ZYXWVUTSRQPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `7`.
    public static final int GLYPH_SEVEN = 34;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 35;

    /// Prevents instantiation.
    private GposChainTriacontaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the digits `1`–`7` and the contiguous `A`–`Z` range.
    ...
    /// Writes ChainContextPos format 1 for `7654321ZYXWVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(94).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 88);
        buffer.putShort((short) 1);
        buffer.putShort((short) 8);
        buffer.putShort((short) 1);
        buffer.putShort((short) 4);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain30Back".getBytes(StandardCharsets.US_ASCII);
