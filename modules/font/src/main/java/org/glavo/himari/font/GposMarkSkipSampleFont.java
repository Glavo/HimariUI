...
/// Generates a face whose GDEF marks `B` and GPOS skips it on pair and chain lookups.
///
/// Lookup 0 is type-2 `IgnoreMarks` for `A` plus `C`. Lookup 1 is type-8 `IgnoreMarks` for
/// `A` plus `C` with lookahead `D`. Lookup 2 is the nested type-1 used by the chain and by
/// type-7 lookup 3, which writes the same skip pair already published by lookup 0.
@NotNullByDefault
public final class GposMarkSkipSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-2/`type-7` X-advance applied to `A` before `C`.
    public static final int PAIR_DELTA = 5;

    /// Type-8 X-advance applied to `A` before `C` with lookahead `D`.
    ...
    private GposMarkSkipSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space, `A`, `B`, `C`, and `D`.
    ...
    /// Writes GDEF ClassDef format 2 marking `B`.
    ...
    /// Writes type-2, type-8, nested type-1, and type-7 `IgnoreMarks` lookups.
    private static byte[] gpos() {
        ...
        lookup0.putShort((short) 0x0008);
        lookup0.putShort((short) 1);
        lookup0.putShort((short) 8);
        lookup0.put(pair);
        byte[] nested = singlePos();
        ByteBuffer lookup2 = ByteBuffer.allocate(8 + nested.length).order(ByteOrder.BIG_ENDIAN);
        lookup2.putShort((short) 1);
        lookup2.putShort((short) 0);
        lookup2.putShort((short) 1);
        lookup2.putShort((short) 8);
        lookup2.put(nested);
        byte[] chain = chainPos();
        ByteBuffer lookup1 = ByteBuffer.allocate(8 + chain.length).order(ByteOrder.BIG_ENDIAN);
        lookup1.putShort((short) 8);
        lookup1.putShort((short) 0x0008);
        lookup1.putShort((short) 1);
        lookup1.putShort((short) 8);
        lookup1.put(chain);
        byte[] context = contextPos();
        ByteBuffer lookup3 = ByteBuffer.allocate(8 + context.length).order(ByteOrder.BIG_ENDIAN);
        lookup3.putShort((short) 7);
        lookup3.putShort((short) 0x0008);
        lookup3.putShort((short) 1);
        lookup3.putShort((short) 8);
        lookup3.put(context);
        byte[] first = lookup0.array();
        byte[] second = lookup1.array();
        byte[] third = lookup2.array();
        byte[] fourth = lookup3.array();
        ByteBuffer lookupList = ByteBuffer.allocate(10 + first.length + second.length + third.length + fourth.length)
                .order(ByteOrder.BIG_ENDIAN);
        lookupList.putShort((short) 4);
        lookupList.putShort((short) 10);
        lookupList.putShort((short) (10 + first.length));
        lookupList.putShort((short) (10 + first.length + second.length));
        lookupList.putShort((short) (10 + first.length + second.length + third.length));
        lookupList.put(first);
        lookupList.put(second);
        lookupList.put(third);
        lookupList.put(fourth);
        ByteBuffer feature = ByteBuffer.allocate(10).order(ByteOrder.BIG_ENDIAN);
        feature.putShort((short) 0);
        feature.putShort((short) 3);
        feature.putShort((short) 0);
    ...
    /// Writes ContextPos format 1 for `AC` using nested lookup 2.
    private static byte[] contextPos() {
    ...
    /// Writes ChainContextPos format 1 for `AC` with lookahead `D`.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(34).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
    ...
    /// Writes the nested SinglePos format 1 used by type-7 and type-8.
    ///
    /// Type-7 `putIfAbsent` loses to the type-2 pair already stored under `A` plus `C`.
    ...
    private static byte[] hmtx() {
        ...
        int[] advances = {0, 3, ADVANCE_LETTER, 4, ADVANCE_LETTER, ADVANCE_LETTER};
    ...
    private static byte[] name() {
        byte[] family = "HimariGposSkip".getBytes(StandardCharsets.US_ASCII);
