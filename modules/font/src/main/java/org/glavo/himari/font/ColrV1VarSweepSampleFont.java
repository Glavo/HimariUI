...
/// Generates a TrueType face whose `A` is a COLR v1 glyph filled by `PaintVarSweepGradient`.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. The paint graph is
/// `PaintGlyph` wrapping a one-stop `PaintVarSweepGradient`.
@NotNullByDefault
public final class ColrV1VarSweepSampleFont {
    /// Units per em.
    ...
    private ColrV1VarSweepSampleFont() {
    }

    /// Builds the COLR v1 sweep-gradient sample font.
    ///
    ...
    /// Builds the COLR v1 sweep-gradient sample font image.
    ///
    ...
    /// Writes COLR v1 with `PaintGlyph` filled by a one-stop variable sweep gradient.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarSweep".getBytes(StandardCharsets.US_ASCII);
