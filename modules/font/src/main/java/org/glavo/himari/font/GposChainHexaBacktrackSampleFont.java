...
/// only after backtrack `IHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainHexaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `IHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `I`.
    public static final int GLYPH_I = 10;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 11;

    /// Prevents instantiation.
    private GposChainHexaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`I` range.
    ...
    /// Writes ChainContextPos format 1 for `IHGFEDABC`. Coverage sits after the rule.
    ...
    private static byte[] name() {
        byte[] family = "HimariChain6Back".getBytes(StandardCharsets.US_ASCII);
