...
/// Generates a TrueType face with one `wght` axis and a `VVAR` advance-height delta for `A`.
///
/// The default vertical advance is [`#DEFAULT_HEIGHT`]. At peak weight `900` `VVAR` adds
/// [`#HEIGHT_DELTA`]. Horizontal `hmtx` stays at [`#DEFAULT_ADVANCE`].
@NotNullByDefault
public final class VvarSampleFont {
    /// `wght` tag.
    ...
    /// Default `vmtx` advance of `A`.
    public static final int DEFAULT_HEIGHT = 10;

    /// Peak `VVAR` height delta of `A`.
    public static final int HEIGHT_DELTA = 6;
    ...
    private VvarSampleFont() {
    }

    /// Builds the VVAR sample font.
    ///
    ...
    /// Builds the VVAR sample font image.
    ///
    ...
    public static MemorySegment bytes() {
        ...
        tables.put("VVAR", vvar());
        tables.put("head", head());
        tables.put("hhea", hhea());
        tables.put("hmtx", hmtx());
        tables.put("vhea", vhea());
        tables.put("vmtx", vmtx());
    ...
    /// Writes one implicit-map `VVAR` with a peak height delta for `A`.
    private static byte[] vvar() {
        ByteBuffer buffer = ByteBuffer.allocate(52).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 0);
        buffer.putInt(20);
        buffer.putInt(0);
    ...
    private static byte[] hhea() {
        return metricHeader(DEFAULT_ADVANCE);
    }

    /// Writes the vhea table.
    private static byte[] vhea() {
        return metricHeader(DEFAULT_HEIGHT);
    }

    /// Writes `hhea` or `vhea` with `numberOfLongMetrics` at offset 34.
    private static byte[] metricHeader(int maxAdvance) {
        ByteBuffer buffer = ByteBuffer.allocate(36).order(ByteOrder.BIG_ENDIAN);
        buffer.putInt(0x00010000);
        buffer.putShort((short) maxAdvance);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) maxAdvance);
        buffer.putShort((short) 0);
    ...
    /// Writes the vmtx table.
    private static byte[] vmtx() {
        ByteBuffer buffer = ByteBuffer.allocate(GLYPH_COUNT * 4).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) DEFAULT_HEIGHT);
        buffer.putShort((short) 0);
        return buffer.array();
    }
    ...
    private static byte[] name() {
        byte[] family = "HimariVvar".getBytes(StandardCharsets.US_ASCII);
