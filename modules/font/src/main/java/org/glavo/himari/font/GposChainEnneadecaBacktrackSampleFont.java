...
/// only after backtrack `VUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainEnneadecaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `VUTSRQPONMLKJIHGFEDABC`.
    ...
    private GposChainEnneadecaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`V` range.
    ...
    /// Writes ChainContextPos format 1 for `VUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(72).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain19Back".getBytes(StandardCharsets.US_ASCII);
