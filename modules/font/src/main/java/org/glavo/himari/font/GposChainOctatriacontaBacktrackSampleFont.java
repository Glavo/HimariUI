...
@NotNullByDefault
public final class GposChainOctatriacontaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `>??>=<;:987654321ZYXWVUTSRQPONMLKJIHGFEDABC`.
    ...
    private GposChainOctatriacontaBacktrackSampleFont() {
    }
    ...
    private static byte[] cmap() {
        ByteBuffer buffer = ByteBuffer.allocate(110).order(ByteOrder.BIG_ENDIAN);
    ...
    /// Writes thirty-eight rectangles.
    ...
    private static byte[] name() {
        byte[] family = "HimariChain38Back".getBytes(StandardCharsets.US_ASCII);
