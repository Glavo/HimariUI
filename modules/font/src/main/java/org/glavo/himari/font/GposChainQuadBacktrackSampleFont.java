...
/// only after backtrack `GFED` and before `BC`.
@NotNullByDefault
public final class GposChainQuadBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `GFEDABC`.
    ...
    private GposChainQuadBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`G` range.
    ...
    /// Writes ChainContextPos format 1 for `GFEDABC`. Coverage sits after the rule.
    ...
    private static byte[] name() {
        byte[] family = "HimariChain4Back".getBytes(StandardCharsets.US_ASCII);
