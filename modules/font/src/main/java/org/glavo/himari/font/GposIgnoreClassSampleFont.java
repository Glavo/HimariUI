...
/// Generates a face whose GPOS pairs skip a GDEF base or ligature between `A` and `C`.
///
/// Lookup 0 uses `IgnoreBaseGlyphs` so `ABC` applies [`#BASE_DELTA`]. Lookup 1 uses
/// `IgnoreLigatures` so `ADC` applies [`#LIGA_DELTA`].
@NotNullByDefault
public final class GposIgnoreClassSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// X-advance applied to `A` when `B` is skipped as a base.
    public static final int BASE_DELTA = 4;

    /// X-advance applied to `A` when `D` is skipped as a ligature.
    public static final int LIGA_DELTA = 9;
    ...
    private GposIgnoreClassSampleFont() {
    }
    ...
    /// Writes type-2 `IgnoreBaseGlyphs` and `IgnoreLigatures` pairs for `AC`.
    private static byte[] gpos() {
        byte[] basePair = pairPos(BASE_DELTA);
        ByteBuffer lookup0 = ByteBuffer.allocate(8 + basePair.length).order(ByteOrder.BIG_ENDIAN);
        ...
        lookup0.put(basePair);
        byte[] ligaPair = pairPos(LIGA_DELTA);
        ByteBuffer lookup1 = ByteBuffer.allocate(8 + ligaPair.length).order(ByteOrder.BIG_ENDIAN);
        lookup1.putShort((short) 2);
        lookup1.putShort((short) GdefTable.FLAG_IGNORE_LIGATURE);
        lookup1.putShort((short) 1);
        lookup1.putShort((short) 8);
        lookup1.put(ligaPair);
    ...
    private static byte[] pairPos(int delta) {
    ...
    private static byte[] hmtx() {
        ...
        int[] advances = {0, 3, ADVANCE_LETTER, ADVANCE_LETTER, ADVANCE_LETTER, ADVANCE_LETTER};
    ...
    private static byte[] name() {
        byte[] family = "HimariGposClass".getBytes(StandardCharsets.US_ASCII);
