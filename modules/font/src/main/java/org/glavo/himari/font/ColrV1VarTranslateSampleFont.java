...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintVarTranslate` of one solid glyph.
///
/// Glyph 2 is the empty base. Glyph 3 is the red rectangle. The paint graph is
/// `PaintVarTranslate` wrapping `PaintGlyph`/`PaintSolid`.
@NotNullByDefault
public final class ColrV1VarTranslateSampleFont {
    /// Units per em.
    ...
    private ColrV1VarTranslateSampleFont() {
    }

    /// Builds the COLR v1 `PaintVarTranslate` sample font.
    ///
    ...
    /// Builds the COLR v1 `PaintVarTranslate` sample font image.
    ///
    ...
    /// Writes COLR v1 with a `PaintVarTranslate` wrapping one solid glyph.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarTranslate".getBytes(StandardCharsets.US_ASCII);
