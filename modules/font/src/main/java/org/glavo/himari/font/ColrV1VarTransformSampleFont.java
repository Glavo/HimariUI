...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintVarTransform` of one solid glyph.
///
/// Glyph 2 is the empty base. Glyph 3 is the red rectangle. The paint graph is
/// `PaintVarTransform` wrapping `PaintGlyph`/`PaintSolid`.
@NotNullByDefault
public final class ColrV1VarTransformSampleFont {
    /// Units per em.
    ...
    private ColrV1VarTransformSampleFont() {
    }

    /// Builds the COLR v1 `PaintVarTransform` sample font.
    ///
    ...
    /// Builds the COLR v1 `PaintVarTransform` sample font image.
    ///
    ...
    /// Writes COLR v1 with a `PaintVarTransform` wrapping one solid glyph.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarTransform".getBytes(StandardCharsets.US_ASCII);
