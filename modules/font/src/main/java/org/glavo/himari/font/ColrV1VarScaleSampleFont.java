...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintVarScale` of one solid glyph.
///
/// Glyph 2 is the empty base. Glyph 3 is the red rectangle. The paint graph is
/// `PaintVarScale` wrapping `PaintGlyph`/`PaintSolid`.
@NotNullByDefault
public final class ColrV1VarScaleSampleFont {
    /// Units per em.
    ...
    private ColrV1VarScaleSampleFont() {
    }

    /// Builds the COLR v1 `PaintVarScale` sample font.
    ///
    ...
    /// Builds the COLR v1 `PaintVarScale` sample font image.
    ///
    ...
    /// Writes COLR v1 with a `PaintVarScale` wrapping one solid glyph.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarScale".getBytes(StandardCharsets.US_ASCII);
