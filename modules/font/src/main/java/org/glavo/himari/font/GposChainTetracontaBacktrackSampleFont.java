...
/// Generates a face whose GPOS type-8 format-1 rule applies [`#CHAIN_DELTA`] to `A`
/// only after backtrack `[@?>=<;:987654321ZYXWVUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainTetracontaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` after backtrack `>?@[[@?>=<;:987654321ZYXWVUTSRQPONMLKJIHGFEDABC`.
    ...
    public static final int GLYPH_ONE = 28;

    /// Twenty-fifth backtrack glyph of `2`.
    public static final int GLYPH_TWO = 29;

    /// Twenty-sixth backtrack glyph of `3`.
    public static final int GLYPH_THREE = 30;

    /// Twenty-seventh backtrack glyph of `4`.
    public static final int GLYPH_FOUR = 31;

    /// Twenty-eighth backtrack glyph of `5`.
    public static final int GLYPH_FIVE = 32;

    /// Twenty-ninth backtrack glyph of `6`.
    public static final int GLYPH_SIX = 33;

    /// Thirtieth backtrack glyph of `7`.
    public static final int GLYPH_SEVEN = 34;

    /// Thirty-first backtrack glyph of `8`.
    public static final int GLYPH_EIGHT = 35;

    /// Thirty-second backtrack glyph of `9`.
    public static final int GLYPH_NINE = 36;

    /// Thirty-third backtrack glyph of `:`.
    public static final int GLYPH_COLON = 37;

    /// Thirty-fourth backtrack glyph of `;`.
    public static final int GLYPH_SEMICOLON = 38;

    /// Thirty-fifth backtrack glyph of `<`.
    public static final int GLYPH_LESS = 39;

    /// Thirty-sixth backtrack glyph of `=`.
    public static final int GLYPH_EQUAL = 40;

    /// Thirty-seventh backtrack glyph of `>`.
    public static final int GLYPH_GREATER = 41;

    /// Thirty-eighth backtrack glyph of `?`.
    public static final int GLYPH_QUESTION = 42;

    /// Thirty-ninth backtrack glyph of `@`.
    public static final int GLYPH_AT = 43;

    /// Farthest backtrack glyph of `[`.
    public static final int GLYPH_BRACKET = 44;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 45;

    /// Prevents instantiation.
    private GposChainTetracontaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space, digits `1`–`9` and `:`/`;`/`<`/`=`/`>`/`?`/`@`, the contiguous `A`–`Z` range, and `[`.
    private static byte[] cmap() {
        ...
        buffer.putShort((short) (GLYPH_ONE - '1'));
        buffer.putShort((short) (GLYPH_A - 'A'));
        buffer.putShort((short) (GLYPH_BRACKET - '['));
        buffer.putShort((short) 1);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
    ...
    /// Writes lookup 0 (type 8) and lookup 1 (type 1).
    ...
    /// Writes ChainContextPos format 1 for `>?@[[@?>=<;:987654321ZYXWVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(114).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 108);
        buffer.putShort((short) 1);
        buffer.putShort((short) 8);
        buffer.putShort((short) 1);
        buffer.putShort((short) 4);
        ...
        buffer.putShort((short) GLYPH_QUESTION);
        buffer.putShort((short) GLYPH_AT);
        buffer.putShort((short) GLYPH_BRACKET);
        buffer.putShort((short) 2);
    ...
    private static byte[] hmtx() {
        ...
        for (int index = 0; index < GLYPH_COUNT; index++) {
            buffer.putShort((short) (index < 2 ? (index == 0 ? 0 : 3) : ADVANCE_LETTER));
            buffer.putShort((short) 0);
        }
        return buffer.array();
    }
    ...
    private static byte[] name() {
        byte[] family = "HimariChain40Back".getBytes(StandardCharsets.US_ASCII);
