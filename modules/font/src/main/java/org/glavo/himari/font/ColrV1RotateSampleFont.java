...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintRotate` of one solid glyph.
///
/// Glyph 2 is the empty base. Glyph 3 is the red rectangle. The paint graph is
/// `PaintRotate` wrapping `PaintGlyph`/`PaintSolid`.
@NotNullByDefault
public final class ColrV1RotateSampleFont {
    /// Units per em.
    ...
    private ColrV1RotateSampleFont() {
    }

    /// Builds the COLR v1 rotate sample font.
    ///
    ...
    /// Builds the COLR v1 rotate sample font image.
    ///
    ...
    /// Writes COLR v1 with a `PaintRotate` wrapping one solid glyph.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrRotate".getBytes(StandardCharsets.US_ASCII);
