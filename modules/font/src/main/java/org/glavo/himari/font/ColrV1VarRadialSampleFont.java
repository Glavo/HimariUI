...
/// Generates a TrueType face whose `A` is a COLR v1 glyph filled by `PaintVarRadialGradient`.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. The paint graph is
/// `PaintGlyph` wrapping a one-stop `PaintVarRadialGradient`.
@NotNullByDefault
public final class ColrV1VarRadialSampleFont {
    /// Units per em.
    ...
    private ColrV1VarRadialSampleFont() {
    }

    /// Builds the COLR v1 radial-gradient sample font.
    ///
    ...
    /// Builds the COLR v1 radial-gradient sample font image.
    ///
    ...
    /// Writes COLR v1 with `PaintGlyph` filled by a one-stop variable radial gradient.
    private static byte[] colr() {
        ...
        buffer.putShort((short) 8);
        buffer.putInt(-1);
        putOffset24At(buffer, fillPos, gradient - glyphPaint);
        ...
        putOffset24At(buffer, linePos, colorLine - gradient);
        return slice(buffer);
    }
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarRadial".getBytes(StandardCharsets.US_ASCII);
