...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintVarScaleUniformAroundCenter` with a store.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. Default uniform scale is
/// `0x4000` (`1.0` as `F2DOT14`). Peak `wght` applies an ItemVariationStore delta of `3` so
/// both [`ColorLayer#scaleX()`] and [`ColorLayer#scaleY()`] are `0x4003`.
@NotNullByDefault
public final class ColrV1VarScaleUniformAroundCenterDeltaSampleFont {
    /// `wght` tag.
    ...
    /// Default uniform scale before variation, as `F2DOT14` `1.0`.
    public static final int BASE_SCALE = 0x4000;

    /// ItemVariationStore delta applied to `scaleX` at peak `wght`.
    public static final int SCALE_DELTA = 3;
    ...
    private ColrV1VarScaleUniformAroundCenterDeltaSampleFont() {
    }

    /// Builds the COLR v1 `PaintVarScaleUniform` variation sample font.
    ///
    ...
    /// Builds the COLR v1 `PaintVarScaleUniform` variation sample font image.
    ///
    ...
    /// Writes COLR v1 with `PaintVarScaleUniform` wrapping `PaintGlyph`/`PaintSolid` and a store.
    private static byte[] colr() {
        ByteBuffer buffer = ByteBuffer.allocate(160).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 0);
        buffer.putInt(0);
        buffer.putInt(0);
        ...
        int scale = buffer.position();
        buffer.put((byte) 23);
        int childPos = buffer.position();
        putOffset24(buffer, 0);
        buffer.putShort((short) BASE_SCALE);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        buffer.putInt(0);
        putOffset24At(buffer, paintRelPos, scale - baseList);
        int glyphPaint = buffer.position();
        buffer.put((byte) 10);
        int fillPos = buffer.position();
        putOffset24(buffer, 0);
        buffer.putShort((short) GLYPH_BACK);
        putOffset24At(buffer, childPos, glyphPaint - scale);
        int solid = buffer.position();
        buffer.put((byte) 2);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0x4000);
        putOffset24At(buffer, fillPos, solid - glyphPaint);
        int store = buffer.position();
        buffer.putInt(storePos, store);
        buffer.putShort((short) 1);
        buffer.putInt(12);
        buffer.putShort((short) 1);
        buffer.putInt(22);
        ...
        buffer.putShort((short) 1);
        buffer.putShort((short) 0);
        buffer.put((byte) SCALE_DELTA);
        return slice(buffer);
    }

    /// Writes one `wght` axis from 100 to 900, default 400.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarSu".getBytes(StandardCharsets.US_ASCII);
