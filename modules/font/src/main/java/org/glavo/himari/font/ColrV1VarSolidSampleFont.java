...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintVarSolid` of one glyph.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. The default instance uses
/// palette 0 (red). Peak `wght` applies an ItemVariationStore delta of `1` so the layer
/// becomes palette 1 (blue).
@NotNullByDefault
public final class ColrV1VarSolidSampleFont {
    /// `wght` tag.
    ...
    private ColrV1VarSolidSampleFont() {
    }

    /// Builds the COLR v1 `PaintVarSolid` sample font.
    ///
    ...
    /// Builds the COLR v1 `PaintVarSolid` sample font image.
    ///
    ...
    /// Writes COLR v1 with `PaintGlyph` filled by `PaintVarSolid` and a one-region store.
    private static byte[] colr() {
        ...
        int varSolid = buffer.position();
        buffer.put((byte) 3);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0x4000);
        buffer.putInt(0);
        putOffset24At(buffer, childPos, varSolid - glyphPaint);
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarSolid".getBytes(StandardCharsets.US_ASCII);
