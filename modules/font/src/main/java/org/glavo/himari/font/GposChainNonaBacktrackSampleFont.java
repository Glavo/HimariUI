...
/// only after backtrack `LKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainNonaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `LKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `L`.
    public static final int GLYPH_L = 13;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 14;

    /// Prevents instantiation.
    private GposChainNonaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`K` range.
    ...
    /// Writes ChainContextPos format 1 for `LKJIHGFEDABC`. Coverage sits after the rule.
    ...
    private static byte[] name() {
        byte[] family = "HimariChain9Back".getBytes(StandardCharsets.US_ASCII);
