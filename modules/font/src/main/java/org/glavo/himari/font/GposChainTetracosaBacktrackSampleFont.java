...
/// only after backtrack `1ZYXWVUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainTetracosaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `1ZYXWVUTSRQPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `1`.
    public static final int GLYPH_ONE = 28;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 29;

    /// Prevents instantiation.
    private GposChainTetracosaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the digit `1` and the contiguous `A`–`Z` range.
    ...
    /// Writes ChainContextPos format 1 for `1ZYXWVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(82).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain24Back".getBytes(StandardCharsets.US_ASCII);
