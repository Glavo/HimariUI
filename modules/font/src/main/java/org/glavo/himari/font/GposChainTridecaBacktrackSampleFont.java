...
/// only after backtrack `PONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainTridecaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `PONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `P`.
    public static final int GLYPH_P = 17;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 18;

    /// Prevents instantiation.
    private GposChainTridecaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`P` range.
    ...
    /// Writes ChainContextPos format 1 for `PONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(60).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 54);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain13Back".getBytes(StandardCharsets.US_ASCII);
