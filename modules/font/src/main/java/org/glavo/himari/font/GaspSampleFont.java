...
/// Generates a face whose `gasp` table withholds grayscale at or below 8 ppem.
@NotNullByDefault
public final class GaspSampleFont {
    ...
    /// Inclusive ppem that still forbids grayscale.
    public static final int BINARY_PPEM = 8;

    /// A ppem that permits grayscale.
    public static final int GRAY_PPEM = 16;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 3;

    /// Prevents instantiation.
    private GaspSampleFont() {
    }
    ...
    /// Writes `gasp` version 1: binary through 8 ppem, grayscale above.
    private static byte[] gasp() {
        ByteBuffer buffer = ByteBuffer.allocate(12).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 2);
        buffer.putShort((short) BINARY_PPEM);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0xFFFF);
        buffer.putShort((short) GaspTable.DOGRAY);
        return buffer.array();
    }

    /// Writes a right triangle that produces partial coverage when grayscale is allowed.
    ...
    private static byte[] name() {
        byte[] family = "HimariGasp".getBytes(StandardCharsets.US_ASCII);
