...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintTransform` of one solid glyph.
///
/// Glyph 2 is the empty base. Glyph 3 is the red rectangle. The paint graph is
/// `PaintTransform` wrapping `PaintGlyph`/`PaintSolid`.
@NotNullByDefault
public final class ColrV1TransformSampleFont {
    /// Units per em.
    ...
    private ColrV1TransformSampleFont() {
    }

    /// Builds the COLR v1 transform sample font.
    ///
    ...
    /// Builds the COLR v1 transform sample font image.
    ///
    ...
    /// Writes COLR v1 with a `PaintTransform` wrapping one solid glyph.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrTransform".getBytes(StandardCharsets.US_ASCII);
