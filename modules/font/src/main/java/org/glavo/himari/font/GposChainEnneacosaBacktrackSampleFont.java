...
/// only after backtrack `654321ZYXWVUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainEnneacosaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `654321ZYXWVUTSRQPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `6`.
    public static final int GLYPH_SIX = 33;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 34;

    /// Prevents instantiation.
    private GposChainEnneacosaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the digits `1`–`6` and the contiguous `A`–`Z` range.
    ...
    /// Writes ChainContextPos format 1 for `654321ZYXWVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(92).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain29Back".getBytes(StandardCharsets.US_ASCII);
