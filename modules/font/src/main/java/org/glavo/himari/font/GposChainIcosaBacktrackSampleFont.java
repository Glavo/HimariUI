...
/// only after backtrack `WVUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainIcosaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `WVUTSRQPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `W`.
    public static final int GLYPH_W = 24;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 25;

    /// Prevents instantiation.
    private GposChainIcosaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`W` range.
    ...
    /// Writes ChainContextPos format 1 for `WVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(74).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 68);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain20Back".getBytes(StandardCharsets.US_ASCII);
