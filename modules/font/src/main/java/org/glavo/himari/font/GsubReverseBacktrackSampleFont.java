...
/// Generates a face whose GSUB type-8 rule turns `A` into `X` after `B` and before `C`.
@NotNullByDefault
public final class GsubReverseBacktrackSampleFont {
    ...
    private GsubReverseBacktrackSampleFont() {
    }
    ...
    /// Writes ReverseChainSingleSubst format 1 with one backtrack and one lookahead.
    ...
    private static byte[] hmtx() {
        ...
        int[] advances = {0, 3, ADVANCE_LETTER, ADVANCE_LETTER, ADVANCE_LETTER, ADVANCE_X};
    ...
    private static byte[] name() {
        byte[] family = "HimariRevBack".getBytes(StandardCharsets.US_ASCII);
