...
/// Generates a face whose GPOS type-7 format-2 class rule applies [`#CONTEXT_DELTA`] to `A`.
///
/// Lookup 0 is the class-based contextual rule listed by `kern`. Lookup 1 is a type-1 X-advance
/// used only as the nested positioning lookup.
@NotNullByDefault
public final class GposContextClassSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-7 X-advance applied to `A` before `B`.
    public static final int CONTEXT_DELTA = 6;
    ...
    private GposContextClassSampleFont() {
    }
    ...
    /// Writes lookup 0 (type 7) and lookup 1 (type 1).
    ...
    /// Writes ContextPos format 2 for `AB` using classes 1 and 2.
    ...
    private static byte[] singlePos() {
        ...
        buffer.putShort((short) CONTEXT_DELTA);
        buffer.putShort((short) 1);
        buffer.putShort((short) 1);
        buffer.putShort((short) GLYPH_A);
        return buffer.array();
    ...
    private static byte[] name() {
        byte[] family = "HimariCtxClass".getBytes(StandardCharsets.US_ASCII);
