...
/// only after backtrack `JIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainHeptaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `JIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `J`.
    public static final int GLYPH_J = 11;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 12;

    /// Prevents instantiation.
    private GposChainHeptaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`J` range.
    ...
    /// Writes ChainContextPos format 1 for `JIHGFEDABC`. Coverage sits after the rule.
    ...
    private static byte[] name() {
        byte[] family = "HimariChain7Back".getBytes(StandardCharsets.US_ASCII);
