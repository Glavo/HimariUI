...
/// Generates a face whose GSUB `calt` type-6 format-2 class rule turns `A` into `Y` before `BC`.
@NotNullByDefault
public final class GsubChainClassSampleFont {
    ...
    private GsubChainClassSampleFont() {
    }
    ...
    /// Writes ChainContextSubst format 2 for `ABC` using input classes 1/2 and lookahead class 1.
    ...
    private static byte[] name() {
        byte[] family = "HimariChainCls".getBytes(StandardCharsets.US_ASCII);
