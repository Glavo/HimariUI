...
/// Generates a TrueType face whose GPOS `curs` feature connects `A` to `B`.
///
/// Glyph 2 is `A` with an exit anchor at [`#EXIT_X`]. Glyph 3 is `B` with an entry
/// anchor at [`#ENTRY_X`]. [`SfntFont#pairAdjustment(int, int)`] therefore returns
/// [`#CURSIVE_DELTA`] for the `AB` pair.
@NotNullByDefault
public final class GposCursiveSampleFont {
    /// Units per em.
    ...
    /// Exit X of `A`.
    public static final int EXIT_X = 8;

    /// Entry X of `B`.
    public static final int ENTRY_X = 2;

    /// Expected `AB` X-advance delta (`exit - entry`).
    public static final int CURSIVE_DELTA = EXIT_X - ENTRY_X;

    /// `curs` feature tag.
    public static final int TAG_CURS = 0x63757273;
    ...
    private GposCursiveSampleFont() {
    }

    /// Builds the cursive sample font.
    ///
    ...
    /// Builds the cursive sample font image.
    ///
    ...
    /// Writes a format-4 cmap for space, `A`, and `B`.
    ...
    /// Writes rectangle outlines for `A` and `B`.
    ...
    /// Writes long `loca` offsets. Glyphs 0–1 are empty.
    ...
    private static byte[] gpos() {
        ...
        feature.putShort((short) 0);
        byte[] featureList = featureList(TAG_CURS, feature.array());
    ...
    /// Writes CursivePos format 1 with an `A` exit and a `B` entry.
    private static byte[] cursive() {
        ...
        buffer.putShort((short) 1);
        buffer.putShort((short) EXIT_X);
        buffer.putShort((short) 0);
        buffer.putShort((short) 1);
        buffer.putShort((short) ENTRY_X);
        buffer.putShort((short) 0);
        return buffer.array();
    }
    ...
    private static byte[] name() {
        byte[] family = "HimariCursive".getBytes(StandardCharsets.US_ASCII);
