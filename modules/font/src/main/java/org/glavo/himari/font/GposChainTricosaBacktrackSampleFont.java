...
/// only after backtrack `ZYXWVUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainTricosaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `ZYXWVUTSRQPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `Z`.
    public static final int GLYPH_Z = 27;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 28;

    /// Prevents instantiation.
    private GposChainTricosaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`Z` range.
    ...
    /// Writes ChainContextPos format 1 for `ZYXWVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(80).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 74);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain23Back".getBytes(StandardCharsets.US_ASCII);
