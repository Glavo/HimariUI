...
/// only after backtrack `21ZYXWVUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainPentacosaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `21ZYXWVUTSRQPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `2`.
    public static final int GLYPH_TWO = 29;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 30;

    /// Prevents instantiation.
    private GposChainPentacosaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the digits `1`–`2` and the contiguous `A`–`Z` range.
    ...
    /// Writes ChainContextPos format 1 for `21ZYXWVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(84).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 78);
        buffer.putShort((short) 1);
        buffer.putShort((short) 8);
        buffer.putShort((short) 1);
        buffer.putShort((short) 4);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain25Back".getBytes(StandardCharsets.US_ASCII);
