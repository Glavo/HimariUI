...
/// Generates a face whose GPOS type-2 lookup uses `MarkAttachmentType` 1 on `A` plus `C`.
///
/// GDEF marks `B` with attach class 2 and `D` with attach class 1. The pair therefore skips `B`
/// and keeps `D`.
@NotNullByDefault
public final class GposMarkAttachSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-2 X-advance applied to `A` before `C` when attach class 1 is kept.
    public static final int ATTACH_DELTA = 7;

    /// `MarkAttachmentType` high byte stored in the lookup flag.
    public static final int ATTACH_TYPE = 1;

    /// Glyph of `A`.
    public static final int GLYPH_A = 2;

    /// Mark glyph of `B` (attach class 2).
    public static final int GLYPH_B = 3;

    /// Glyph of `C`.
    public static final int GLYPH_C = 4;

    /// Mark glyph of `D` (attach class 1).
    ...
    private GposMarkAttachSampleFont() {
    }
    ...
    /// Writes glyph-class marks plus mark-attach classes for `B` and `D`.
    private static byte[] gdef() {
        ...
        buffer.putShort((short) GLYPH_D);
        buffer.putShort((short) GLYPH_D);
        buffer.putShort((short) ATTACH_TYPE);
        return buffer.array();
    }

    /// Writes a type-2 `MarkAttachmentType` 1 pair and a type-7 copy of the same pair.
    private static byte[] gpos() {
        ...
        lookup0.putShort((short) (ATTACH_TYPE << 8));
        ...
        ByteBuffer lookup1 = ByteBuffer.allocate(8 + context.length).order(ByteOrder.BIG_ENDIAN);
        lookup1.putShort((short) 7);
        lookup1.putShort((short) (ATTACH_TYPE << 8));
    ...
    private static byte[] pairPos() {
        ...
        buffer.putShort((short) ATTACH_DELTA);
        buffer.putShort((short) 1);
        buffer.putShort((short) 1);
    ...
    /// Writes the nested SinglePos used only by the type-7 duplicate.
    ...
    private static byte[] hmtx() {
        ...
        int[] advances = {0, 3, ADVANCE_LETTER, 4, ADVANCE_LETTER, 4};
    ...
    private static byte[] name() {
        byte[] family = "HimariGposAttach".getBytes(StandardCharsets.US_ASCII);
