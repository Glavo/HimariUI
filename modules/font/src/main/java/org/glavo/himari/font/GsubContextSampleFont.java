...
/// Generates a face whose GSUB `calt` type-5 rule turns `A` into `X` before `B`.
@NotNullByDefault
public final class GsubContextSampleFont {
    ...
    /// Writes ContextSubst format 1 for `AB`.
    ...
    private static byte[] name() {
        byte[] family = "HimariCalt".getBytes(StandardCharsets.US_ASCII);
