...
/// Generates a face whose GSUB `calt` type-5 format-2 class rule turns `A` into `X` before `B`.
@NotNullByDefault
public final class GsubContextClassSampleFont {
    ...
    private GsubContextClassSampleFont() {
    }
    ...
    /// Writes ContextSubst format 2 for `AB` using classes 1 and 2.
    ...
    private static byte[] name() {
        byte[] family = "HimariCalt2".getBytes(StandardCharsets.US_ASCII);
