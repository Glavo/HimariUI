...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintVarTransform` with a store.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. Default `dx` is `2`. Peak
/// `wght` applies an ItemVariationStore delta of `3` so [`ColorLayer#translateX()`] is `5`.
@NotNullByDefault
public final class ColrV1VarTransformDeltaSampleFont {
    /// `wght` tag.
    ...
    /// Default transform `xx` before variation, as `16.16` `1.0`.
    public static final int BASE_TRANSFORM_XX = 0x00010000;

    /// ItemVariationStore delta applied to `xx` at peak `wght`.
    public static final int TRANSFORM_DELTA = 3;
    ...
    private ColrV1VarTransformDeltaSampleFont() {
    }

    /// Builds the COLR v1 `PaintVarTransform` variation sample font.
    ///
    ...
    /// Builds the COLR v1 `PaintVarTransform` variation sample font image.
    ///
    ...
    /// Writes COLR v1 with `PaintVarTransform` wrapping `PaintGlyph`/`PaintSolid` and a store.
    private static byte[] colr() {
        ...
        int transform = buffer.position();
        buffer.put((byte) 13);
        int childPos = buffer.position();
        putOffset24(buffer, 0);
        buffer.putInt(BASE_TRANSFORM_XX);
        buffer.putInt(0);
        buffer.putInt(0);
        buffer.putInt(BASE_TRANSFORM_XX);
        buffer.putInt(0);
        buffer.putInt(0);
        buffer.putInt(0);
        putOffset24At(buffer, paintRelPos, transform - baseList);
        int glyphPaint = buffer.position();
        buffer.put((byte) 10);
        int fillPos = buffer.position();
        putOffset24(buffer, 0);
        buffer.putShort((short) GLYPH_BACK);
        putOffset24At(buffer, childPos, glyphPaint - transform);
        ...
        buffer.putShort((short) 1);
        buffer.putShort((short) 0);
        buffer.put((byte) TRANSFORM_DELTA);
        return slice(buffer);
    }
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarXx".getBytes(StandardCharsets.US_ASCII);
