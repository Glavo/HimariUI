...
@NotNullByDefault
public final class GsubChainTetracontaBacktrackSampleFont {
    ...
    public static final int GLYPH_QUESTION = 43;

    /// Thirty-ninth backtrack glyph of `@`.
    public static final int GLYPH_AT = 44;

    /// Farthest backtrack glyph of `[`.
    public static final int GLYPH_BRACKET = 45;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 46;

    /// Prevents instantiation.
    private GsubChainTetracontaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space, digits `0`–`9` and `:`/`;`/`<`/`=`/`>`/`?`/`@`, the contiguous `A`–`Z` range, and `[`.
    ...
    private static byte[] name() {
        byte[] family = "HimariGsubChain40Back".getBytes(StandardCharsets.US_ASCII);
