...
/// only after backtrack `321ZYXWVUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainHexacosaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `321ZYXWVUTSRQPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `3`.
    public static final int GLYPH_THREE = 30;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 31;

    /// Prevents instantiation.
    private GposChainHexacosaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the digits `1`–`3` and the contiguous `A`–`Z` range.
    ...
    /// Writes ChainContextPos format 1 for `321ZYXWVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(86).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain26Back".getBytes(StandardCharsets.US_ASCII);
