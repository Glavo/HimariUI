...
/// only after backtrack `ED` and before `BC`.
@NotNullByDefault
public final class GsubChainDoubleBacktrackSampleFont {
    ...
    /// Writes a format-4 cmap for space through `E` and `Y`.
    ...
    /// Writes ChainContextSubst format 2 for `EDABC`.
    ...
    private static byte[] hmtx() {
        ...
        int[] advances = {
            0, 3, ADVANCE_LETTER, ADVANCE_LETTER, ADVANCE_LETTER, ADVANCE_LETTER, ADVANCE_LETTER, ADVANCE_Y
        };
    ...
    private static byte[] name() {
        byte[] family = "HimariGsub2Back".getBytes(StandardCharsets.US_ASCII);
