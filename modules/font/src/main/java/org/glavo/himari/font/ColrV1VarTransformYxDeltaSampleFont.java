...
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. Default `yx` is `2`. Peak
/// `wght` applies an ItemVariationStore delta of `3` so [`ColorLayer#transformYx()`] is `5`.
@NotNullByDefault
public final class ColrV1VarTransformYxDeltaSampleFont {
    /// `wght` tag.
    ...
    /// Default transform `yx` before variation, as `16.16` units.
    public static final int BASE_TRANSFORM_YX = 2;

    /// ItemVariationStore delta applied to `yx` at peak `wght`.
    ...
    private ColrV1VarTransformYxDeltaSampleFont() {
    }
    ...
    private static byte[] colr() {
        ...
        putOffset24(buffer, 0);
        buffer.putInt(0);
        buffer.putInt(BASE_TRANSFORM_YX);
        buffer.putInt(0);
        buffer.putInt(0);
        buffer.putInt(0);
        buffer.putInt(0);
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarYx".getBytes(StandardCharsets.US_ASCII);
