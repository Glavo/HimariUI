...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintVarTranslate` with a store.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. Default `dy` is `2`. Peak
/// `wght` applies an ItemVariationStore delta of `3` to the second variable field so
/// [`ColorLayer#translateY()`] is `5`.
@NotNullByDefault
public final class ColrV1VarTranslateYDeltaSampleFont {
    /// `wght` tag.
    ...
    /// Default `dy` before variation.
    public static final int BASE_TRANSLATE_Y = 2;

    /// ItemVariationStore delta applied to `dy` at peak `wght`.
    public static final int TRANSLATE_Y_DELTA = 3;
    ...
    private ColrV1VarTranslateYDeltaSampleFont() {
    }

    /// Builds the COLR v1 `PaintVarTranslate` variation sample font.
    ///
    ...
    /// Builds the COLR v1 `PaintVarTranslate` variation sample font image.
    ///
    ...
    /// Writes COLR v1 with `PaintVarTranslate` wrapping `PaintGlyph`/`PaintSolid` and a store.
    private static byte[] colr() {
        ...
        putOffset24(buffer, 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) BASE_TRANSLATE_Y);
        buffer.putInt(0);
        ...
        buffer.putShort((short) 0);
        buffer.put((byte) 0);
        buffer.put((byte) TRANSLATE_Y_DELTA);
        return slice(buffer);
    }
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarDy".getBytes(StandardCharsets.US_ASCII);
