...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintTranslate` of one solid glyph.
///
/// Glyph 2 is the empty base. Glyph 3 is the red rectangle. The paint graph is
/// `PaintTranslate` wrapping `PaintGlyph`/`PaintSolid`.
@NotNullByDefault
public final class ColrV1TranslateSampleFont {
    /// Units per em.
    ...
    private ColrV1TranslateSampleFont() {
    }

    /// Builds the COLR v1 translate sample font.
    ///
    ...
    /// Builds the COLR v1 translate sample font image.
    ///
    ...
    /// Writes COLR v1 with a `PaintTranslate` wrapping one solid glyph.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrTranslate".getBytes(StandardCharsets.US_ASCII);
