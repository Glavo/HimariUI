...
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. Default `xy` is `2`. Peak
/// `wght` applies an ItemVariationStore delta of `3` so [`ColorLayer#transformXy()`] is `5`.
@NotNullByDefault
public final class ColrV1VarTransformXyDeltaSampleFont {
    /// `wght` tag.
    ...
    /// Default transform `xy` before variation, as `16.16` units.
    public static final int BASE_TRANSFORM_XY = 2;

    /// ItemVariationStore delta applied to `xy` at peak `wght`.
    ...
    private ColrV1VarTransformXyDeltaSampleFont() {
    }
    ...
    private static byte[] colr() {
        ...
        buffer.putInt(0);
        buffer.putInt(0);
        buffer.putInt(BASE_TRANSFORM_XY);
        buffer.putInt(0);
        buffer.putInt(0);
        buffer.putInt(0);
        buffer.putInt(0);
