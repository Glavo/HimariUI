...
/// Generates a TrueType face with one `wght` axis and an `HVAR` advance delta for `A`.
///
/// The default advance is [`#DEFAULT_ADVANCE`]. At peak weight `900` `HVAR` adds
/// [`#ADVANCE_DELTA`]. There is no `gvar`, so outline points stay put.
@NotNullByDefault
public final class HvarSampleFont {
    /// `wght` tag.
    ...
    /// Peak `HVAR` advance delta of `A`.
    public static final int ADVANCE_DELTA = 12;
    ...
    private HvarSampleFont() {
    }

    /// Builds the HVAR sample font.
    ///
    ...
    /// Builds the HVAR sample font image.
    ///
    ...
    /// Writes one implicit-map `HVAR` with a peak advance delta for `A`.
    private static byte[] hvar() {
    ...
    private static byte[] name() {
        byte[] family = "HimariHvar".getBytes(StandardCharsets.US_ASCII);
