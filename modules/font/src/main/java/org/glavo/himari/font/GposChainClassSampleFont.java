...
///
/// Lookup 0 is the class-based chain rule listed by `kern`. Lookup 1 is a nested type-1 X-advance.
@NotNullByDefault
public final class GposChainClassSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` before `B` with lookahead `C`.
    ...
    private GposChainClassSampleFont() {
    }
    ...
    /// Writes ChainContextPos format 2 for `ABC` using input classes 1/2 and lookahead class 1.
    ...
    private static byte[] name() {
        byte[] family = "HimariChainClass".getBytes(StandardCharsets.US_ASCII);
