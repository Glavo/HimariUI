...
/// Generates a face whose `MarkAttachmentType` type-8 rule applies [`#CHAIN_DELTA`] to `A`
/// only after backtrack `D`, skipping mark `B` before `CE`.
@NotNullByDefault
public final class GposAttachChainBacktrackSampleFont {
    ...
    /// Mark glyph skipped between `A` and `C`.
    ...
    private GposAttachChainBacktrackSampleFont() {
    }
    ...
    /// Marks `B` as a GDEF mark with attach class 2.
    ...
    /// Writes type-8 `MarkAttachmentType` plus nested type-1.
    ...
    private static byte[] name() {
        byte[] family = "HimariAttBack".getBytes(StandardCharsets.US_ASCII);
