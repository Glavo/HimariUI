...
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. Default `dy` is `2`. Peak
/// `wght` applies an ItemVariationStore delta of `3` so [`ColorLayer#transformDy()`] is `5`.
@NotNullByDefault
public final class ColrV1VarTransformDyDeltaSampleFont {
    /// `wght` tag.
    ...
    /// Default transform `dy` before variation, as `16.16` units.
    public static final int BASE_TRANSFORM_DY = 2;
    ...
    private ColrV1VarTransformDyDeltaSampleFont() {
    }
    ...
    private static byte[] colr() {
        ...
        buffer.putInt(0);
        buffer.putInt(0);
        buffer.putInt(BASE_TRANSFORM_DY);
        buffer.putInt(0);
