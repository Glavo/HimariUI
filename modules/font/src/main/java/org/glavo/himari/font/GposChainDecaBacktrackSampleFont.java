...
/// only after backtrack `MLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainDecaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `MLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `M`.
    public static final int GLYPH_M = 14;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 15;

    /// Prevents instantiation.
    private GposChainDecaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`M` range.
    ...
    /// Writes ChainContextPos format 1 for `MLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(54).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
        buffer.putShort((short) 48);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain10Back".getBytes(StandardCharsets.US_ASCII);
