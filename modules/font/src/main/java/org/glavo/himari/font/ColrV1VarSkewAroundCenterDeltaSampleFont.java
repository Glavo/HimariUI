...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintVarSkew` with a store.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. Default `xSkewAngle` is `2`
/// as `F2DOT14`. Peak `wght` applies an ItemVariationStore delta of `3` so
/// [`ColorLayer#skewY()`] is `5`.
@NotNullByDefault
public final class ColrV1VarSkewAroundCenterDeltaSampleFont {
    /// `wght` tag.
    ...
    /// Default around-center X skew angle before variation, as `F2DOT14`.
    public static final int BASE_SKEW_X = 2;

    /// ItemVariationStore delta applied to the X skew at peak `wght`.
    public static final int SKEW_DELTA = 3;
    ...
    private ColrV1VarSkewAroundCenterDeltaSampleFont() {
    }

    /// Builds the COLR v1 `PaintVarSkew` variation sample font.
    ///
    ...
    /// Builds the COLR v1 `PaintVarSkew` variation sample font image.
    ///
    ...
    /// Writes COLR v1 with `PaintVarSkew` wrapping `PaintGlyph`/`PaintSolid` and a store.
    private static byte[] colr() {
        ...
        int skew = buffer.position();
        buffer.put((byte) 31);
        int childPos = buffer.position();
        putOffset24(buffer, 0);
        buffer.putShort((short) BASE_SKEW_X);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        buffer.putInt(0);
        putOffset24At(buffer, paintRelPos, skew - baseList);
        int glyphPaint = buffer.position();
        buffer.put((byte) 10);
        int fillPos = buffer.position();
        putOffset24(buffer, 0);
        buffer.putShort((short) GLYPH_BACK);
        putOffset24At(buffer, childPos, glyphPaint - skew);
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarSkewY".getBytes(StandardCharsets.US_ASCII);
