...
/// Generates a face whose GDEF marks `B` and `rlig` ligates `A` plus `C` with `IgnoreMarks`.
@NotNullByDefault
public final class GdefMarkSampleFont {
    ...
    /// Writes type-4 `rlig` with `IgnoreMarks`.
    private static byte[] gsub() {
        byte[] subst = ligatureSubst();
        ByteBuffer lookup0 = ByteBuffer.allocate(8 + subst.length).order(ByteOrder.BIG_ENDIAN);
        lookup0.putShort((short) 4);
        ...
        feature.putShort((short) 0);
        byte[] featureList = featureList(SfntFont.TAG_RLIG, feature.array());
    ...
    private static byte[] hmtx() {
        ...
        int[] advances = {0, 3, ADVANCE_LETTER, 4, ADVANCE_LETTER, ADVANCE_LIGATURE};
    ...
    private static byte[] name() {
        byte[] family = "HimariGdef".getBytes(StandardCharsets.US_ASCII);
