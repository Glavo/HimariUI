...
/// Generates a TrueType face whose `A` is a COLR v1 base of two `PaintGlyph` solids.
///
/// Glyph 2 is the empty base. Glyph 3 is the red back rectangle. Glyph 4 is the blue front
/// rectangle. The paint graph is `PaintColrLayers` of two `PaintGlyph`/`PaintSolid` pairs.
@NotNullByDefault
public final class ColrV1SampleFont {
    /// Units per em.
    ...
    private ColrV1SampleFont() {
    }

    /// Builds the COLR v1 sample font.
    ///
    ...
    /// Builds the COLR v1 sample font image.
    ///
    ...
    /// Writes COLR v1 with one `PaintColrLayers` base of two solid glyphs.
    private static byte[] colr() {
        ...
        buffer.putInt(0);
        int layerListPos = buffer.position();
        buffer.putInt(0);
        buffer.putInt(0);
        buffer.putInt(0);
        buffer.putInt(0);
        ...
        int layersPaint = buffer.position();
        buffer.put((byte) 1);
        buffer.put((byte) 2);
        buffer.putInt(0);
        putOffset24At(buffer, paintRelPos, layersPaint - baseList);
        int layerList = buffer.position();
        buffer.putInt(layerListPos, layerList);
        buffer.putInt(2);
        int off0 = buffer.position();
        putOffset24(buffer, 0);
        int off1 = buffer.position();
        putOffset24(buffer, 0);
        int paint0 = buffer.position();
        writePaintGlyph(buffer, GLYPH_BACK, 0);
        int paint1 = buffer.position();
        writePaintGlyph(buffer, GLYPH_FRONT, 1);
        putOffset24At(buffer, off0, paint0 - layerList);
        putOffset24At(buffer, off1, paint1 - layerList);
        return slice(buffer);
    }
    ...
    private static byte[] name() {
        byte[] family = "HimariColrV1".getBytes(StandardCharsets.US_ASCII);
