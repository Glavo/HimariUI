...
/// only after backtrack `TSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainHeptadecaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `TSRQPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `T`.
    public static final int GLYPH_T = 21;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 22;

    /// Prevents instantiation.
    private GposChainHeptadecaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`T` range.
    ...
    /// Writes ChainContextPos format 1 for `TSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(68).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain17Back".getBytes(StandardCharsets.US_ASCII);
