...
/// only after backtrack `QPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainTetradecaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `QPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `Q`.
    public static final int GLYPH_Q = 18;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 19;

    /// Prevents instantiation.
    private GposChainTetradecaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`Q` range.
    ...
    /// Writes ChainContextPos format 1 for `QPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(62).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain14Back".getBytes(StandardCharsets.US_ASCII);
