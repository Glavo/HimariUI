...
/// only after backtrack `KJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainOctaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `KJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `K`.
    public static final int GLYPH_K = 12;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 13;

    /// Prevents instantiation.
    private GposChainOctaBacktrackSampleFont() {
    }
    ...
    /// Writes ChainContextPos format 1 for `KJIHGFEDABC`. Coverage sits after the rule.
    ...
    private static byte[] name() {
        byte[] family = "HimariChain8Back".getBytes(StandardCharsets.US_ASCII);
