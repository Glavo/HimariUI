...
/// Generates a TrueType face whose `A` is a COLR v1 `PaintComposite` of two solid glyphs.
///
/// Glyph 2 is the empty base. Glyph 3 is the red source rectangle. Glyph 4 is the blue
/// backdrop rectangle. The paint graph is `PaintComposite` of two `PaintGlyph`/`PaintSolid`
/// pairs.
@NotNullByDefault
public final class ColrV1CompositeSampleFont {
    /// Units per em.
    ...
    private ColrV1CompositeSampleFont() {
    }

    /// Builds the COLR v1 composite sample font.
    ///
    ...
    /// Builds the COLR v1 composite sample font image.
    ///
    ...
    /// Writes COLR v1 with `PaintComposite` of backdrop then source solids.
    private static byte[] colr() {
        ...
        int composite = buffer.position();
        buffer.put((byte) 32);
        int sourcePos = buffer.position();
        putOffset24(buffer, 0);
        buffer.put((byte) 0);
        int backdropPos = buffer.position();
        putOffset24(buffer, 0);
        putOffset24At(buffer, paintRelPos, composite - baseList);
        int source = buffer.position();
        writePaintGlyph(buffer, GLYPH_BACK, 0);
        int backdrop = buffer.position();
        writePaintGlyph(buffer, GLYPH_FRONT, 1);
        putOffset24At(buffer, sourcePos, source - composite);
        putOffset24At(buffer, backdropPos, backdrop - composite);
        return slice(buffer);
    }
    ...
    private static byte[] name() {
        byte[] family = "HimariColrComposite".getBytes(StandardCharsets.US_ASCII);
