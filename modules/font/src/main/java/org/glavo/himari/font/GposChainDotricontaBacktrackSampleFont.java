...
@NotNullByDefault
public final class GposChainDotricontaBacktrackSampleFont {
    ...
    /// Farthest backtrack glyph of `9`.
    public static final int GLYPH_NINE = 36;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 37;

    /// Prevents instantiation.
    private GposChainDotricontaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the digits `1`–`9` and the contiguous `A`–`Z` range.
    private static byte[] cmap() {
        ByteBuffer buffer = ByteBuffer.allocate(98).order(ByteOrder.BIG_ENDIAN);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain32Back".getBytes(StandardCharsets.US_ASCII);
