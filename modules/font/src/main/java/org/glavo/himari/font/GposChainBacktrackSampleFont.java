...
/// only after backtrack `D` and before `BC`.
@NotNullByDefault
public final class GposChainBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `DABC`.
    ...
    private GposChainBacktrackSampleFont() {
    }
    ...
    /// Writes ChainContextPos format 2 for `DABC` using backtrack class 1.
    ...
    private static byte[] name() {
        byte[] family = "HimariChainBack".getBytes(StandardCharsets.US_ASCII);
