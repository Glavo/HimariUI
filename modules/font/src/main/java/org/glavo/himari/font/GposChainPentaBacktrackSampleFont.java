...
/// only after backtrack `HGFED` and before `BC`.
@NotNullByDefault
public final class GposChainPentaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `HGFEDABC`.
    ...
    private GposChainPentaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`H` range.
    ...
    /// Writes ChainContextPos format 1 for `HGFEDABC`. Coverage sits after the rule.
    ...
    private static byte[] name() {
        byte[] family = "HimariChain5Back".getBytes(StandardCharsets.US_ASCII);
