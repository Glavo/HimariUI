...
/// Generates a face whose GSUB `calt` type-6 format-2 class rule turns `A` into `Y`
/// only after backtrack `FED` and before `BC`.
@NotNullByDefault
public final class GsubChainTripleBacktrackSampleFont {
    ...
    public static final int GLYPH_Y = 8;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 9;

    /// Prevents instantiation.
    private GsubChainTripleBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space through `F` and `Y`.
    ...
    /// Writes ChainContextSubst format 2 for `FEDABC`.
    ...
    private static byte[] hhea() {
        ...
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) ADVANCE_Y);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
    ...
    private static byte[] hmtx() {
        ...
        int[] advances = {0, 3, 10, 10, 10, 10, 10, 10, ADVANCE_Y};
    ...
    private static byte[] name() {
        byte[] family = "HimariGsub3Back".getBytes(StandardCharsets.US_ASCII);
