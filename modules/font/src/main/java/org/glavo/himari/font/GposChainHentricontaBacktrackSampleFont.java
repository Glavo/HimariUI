...
/// only after backtrack `87654321ZYXWVUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainHentricontaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `87654321ZYXWVUTSRQPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `8`.
    public static final int GLYPH_EIGHT = 35;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 36;

    /// Prevents instantiation.
    private GposChainHentricontaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the digits `1`–`8` and the contiguous `A`–`Z` range.
    ...
    /// Writes ChainContextPos format 1 for `87654321ZYXWVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    ...
    private static byte[] name() {
        byte[] family = "HimariChain31Back".getBytes(StandardCharsets.US_ASCII);
