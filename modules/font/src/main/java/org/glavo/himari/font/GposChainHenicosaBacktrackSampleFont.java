...
/// only after backtrack `XWVUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainHenicosaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `XWVUTSRQPONMLKJIHGFEDABC`.
    ...
    private GposChainHenicosaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the contiguous `A`–`X` range.
    ...
    /// Writes twenty-three rectangles.
    ...
    /// Writes ChainContextPos format 1 for `XWVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(76).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain21Back".getBytes(StandardCharsets.US_ASCII);
