...
/// only after backtrack `54321ZYXWVUTSRQPONMLKJIHGFED` and before `BC`.
@NotNullByDefault
public final class GposChainOctacosaBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `54321ZYXWVUTSRQPONMLKJIHGFEDABC`.
    ...
    /// Farthest backtrack glyph of `5`.
    public static final int GLYPH_FIVE = 32;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 33;

    /// Prevents instantiation.
    private GposChainOctacosaBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space and the digits `1`–`5` and the contiguous `A`–`Z` range.
    ...
    /// Writes twenty-eight rectangles.
    ...
    /// Writes ChainContextPos format 1 for `54321ZYXWVUTSRQPONMLKJIHGFEDABC`. Coverage sits after the rule.
    private static byte[] chainPos() {
        ByteBuffer buffer = ByteBuffer.allocate(90).order(ByteOrder.BIG_ENDIAN);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariChain28Back".getBytes(StandardCharsets.US_ASCII);
