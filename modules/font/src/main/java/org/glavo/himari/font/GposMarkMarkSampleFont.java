...
/// Generates a face whose GPOS `mkmk` lookup type 6 attaches kasra to fatha.
///
/// The anchors are `(4, 8)` on fatha and `(1, 1)` on kasra, so the mark offset is
/// [`#MARK_X_OFFSET`], [`#MARK_Y_OFFSET`].
@NotNullByDefault
public final class GposMarkMarkSampleFont {
    /// Expected kasra X offset.
    public static final int MARK_X_OFFSET = 3;

    /// Expected kasra Y offset.
    public static final int MARK_Y_OFFSET = 7;

    /// Fatha glyph (`U+064E`).
    public static final int GLYPH_FATHA = 2;

    /// Kasra glyph (`U+0650`).
    public static final int GLYPH_KASRA = 3;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 4;

    /// Prevents instantiation.
    private GposMarkMarkSampleFont() {
    }

    /// Builds the mark-to-mark sample font.
    ///
    ...
    /// Writes a format-4 cmap for space, fatha, and kasra.
    private static byte[] cmap() {
        ...
        buffer.putShort((short) (GLYPH_KASRA - 0x0650));
        buffer.putShort((short) 1);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
    ...
    private static byte[] gpos() {
        ...
        feature.putShort((short) 0);
        byte[] featureList = featureList(0x6D6B6D6B, feature.array());
    ...
    /// Writes MarkMarkPos format 1 with one class.
    private static byte[] markMark() {
        ...
        buffer.putShort((short) 1);
        buffer.putShort((short) 1);
        buffer.putShort((short) GLYPH_KASRA);
        buffer.putShort((short) 1);
        buffer.putShort((short) 1);
    ...
    private static byte[] name() {
        byte[] family = "HimariMkmk".getBytes(StandardCharsets.US_ASCII);
