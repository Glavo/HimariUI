...
/// Generates a face whose GSUB type-8 reverse rule turns `A` into `X` after `ED` and before `C`.
@NotNullByDefault
public final class GsubReverseDoubleBacktrackSampleFont {
    ...
    public static final int GLYPH_D = 3;

    /// Lookahead glyph of `C`.
    public static final int GLYPH_C = 4;

    /// Substitute glyph.
    public static final int GLYPH_X = 5;
    ...
    private GsubReverseDoubleBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space, `A`, `C`, `D`, `E`, and `X`.
    ...
    /// Writes lookup 0 (type 8).
    private static byte[] gsub() {
        byte[] reverse = reverseSubst();
        ByteBuffer lookup0 = ByteBuffer.allocate(8 + reverse.length).order(ByteOrder.BIG_ENDIAN);
    ...
    /// Writes ReverseChainSingleSubst format 1 with two backtrack coverages and one lookahead.
    private static byte[] reverseSubst() {
    ...
    private static byte[] hmtx() {
        ...
        int[] advances = {0, 3, ADVANCE_LETTER, ADVANCE_LETTER, ADVANCE_LETTER, ADVANCE_X, ADVANCE_LETTER};
    ...
    private static byte[] name() {
        byte[] family = "HimariRev2Back".getBytes(StandardCharsets.US_ASCII);
