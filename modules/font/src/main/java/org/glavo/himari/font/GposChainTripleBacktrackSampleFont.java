...
/// Generates a face whose GPOS type-8 format-2 class rule applies [`#CHAIN_DELTA`] to `A`
/// only after backtrack `FED` and before `BC`.
@NotNullByDefault
public final class GposChainTripleBacktrackSampleFont {
    /// Isolated letter advance.
    public static final int ADVANCE_LETTER = 10;

    /// Type-8 X-advance applied to `A` in `FEDABC`.
    ...
    /// Farthest backtrack glyph of `F`.
    public static final int GLYPH_F = 7;

    /// Glyph count including `.notdef` and space.
    private static final int GLYPH_COUNT = 8;

    /// Prevents instantiation.
    private GposChainTripleBacktrackSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space through `F`.
    private static byte[] cmap() {
        ByteBuffer buffer = ByteBuffer.allocate(144).order(ByteOrder.BIG_ENDIAN);
        ...
        buffer.putShort((short) (GLYPH_F - 'F'));
        buffer.putShort((short) 1);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
        buffer.putShort((short) 0);
    ...
    /// Writes ChainContextPos format 2 for `FEDABC`.
    ...
    private static byte[] name() {
        byte[] family = "HimariChain3Back".getBytes(StandardCharsets.US_ASCII);
