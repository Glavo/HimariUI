...
/// Generates a TrueType face whose `A` is a COLR v1 glyph filled by `PaintVarLinearGradient`.
///
/// Glyph 2 is the empty base. Glyph 3 is the painted rectangle. The paint graph is
/// `PaintGlyph` wrapping a one-stop `PaintVarLinearGradient`.
@NotNullByDefault
public final class ColrV1VarLinearSampleFont {
    /// Units per em.
    ...
    private ColrV1VarLinearSampleFont() {
    }

    /// Builds the COLR v1 `PaintVarLinearGradient` sample font.
    ///
    ...
    /// Builds the COLR v1 gradient sample font image.
    ///
    ...
    /// Writes COLR v1 with `PaintGlyph` filled by a one-stop variable linear gradient.
    ...
    private static byte[] name() {
        byte[] family = "HimariColrVarLinear".getBytes(StandardCharsets.US_ASCII);
