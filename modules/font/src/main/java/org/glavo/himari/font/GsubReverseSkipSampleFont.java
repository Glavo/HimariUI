...
/// Generates a face whose GSUB type-8 `calt` skips a GDEF mark in the lookahead.
///
/// `A` becomes `X` when the first non-mark after `A` is `C`.
@NotNullByDefault
public final class GsubReverseSkipSampleFont {
    ...
    private GsubReverseSkipSampleFont() {
    }
    ...
    /// Writes a format-4 cmap for space, `A`, `B`, `C`, and `X`.
    ...
    /// Writes type-8 `IgnoreMarks` reverse `calt`.
    private static byte[] gsub() {
        ...
        lookup0.putShort((short) GdefTable.FLAG_IGNORE_MARKS);
    ...
    /// Writes ReverseChainSingleSubst format 1 for lookahead `C`.
    ...
    private static byte[] hmtx() {
        ...
        int[] advances = {0, 3, ADVANCE_LETTER, 4, ADVANCE_LETTER, ADVANCE_X};
    ...
    private static byte[] name() {
        byte[] family = "HimariRevSkip".getBytes(StandardCharsets.US_ASCII);
