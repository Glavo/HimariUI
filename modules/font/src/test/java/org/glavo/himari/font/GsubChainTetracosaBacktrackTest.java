...
/// Verifies twenty-four-glyph GSUB chain backtrack through [`SfntFont#chainSubstitute(int[], int, int, int)`].
@NotNullByDefault
final class GsubChainTetracosaBacktrackTest {
    /// Substitutes `A` only when format-1 backtrack `1ZYXWVUTSRQPONMLKJIHGFED` and lookahead `BC` are present.
    @Test
    void format1BacktrackRequiresOneZyxwvutsrqponmlkjihgfed() {
        SfntFont font = GsubChainTetracosaBacktrackSampleFont.create();
        ...
        assertEquals(GsubChainTetracosaBacktrackSampleFont.GLYPH_Y, y);
        assertEquals(GsubChainTetracosaBacktrackSampleFont.GLYPH_Z, z);
        assertEquals(GsubChainTetracosaBacktrackSampleFont.GLYPH_ZERO, zero);
        assertEquals(GsubChainTetracosaBacktrackSampleFont.GLYPH_ONE, one);
        ...
        assertEquals(
                a,
                font.chainSubstitute(
                        new int[] {one, y, x, w, v, u, t, s, r, q, p, o, n, m, l, k, j, i, h, g, f, e, d, a, b, c},
                        23,
                        3,
                        SfntFont.TAG_CALT
                )
