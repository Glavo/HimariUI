...
val schemaFile = layout.projectDirectory.file("src/main/abi/macos-schema-v1.json")
...
val generatedMetadataFile = generatedMetadataDirectory.map {
    it.file("META-INF/native-image/org.glavo.himari/himari-platform-macos/reachability-metadata.json")
}
val sourceSets = extensions.getByType<SourceSetContainer>()
val conformanceDirectory = rootProject.layout.buildDirectory.dir("conformance/m7-macos")
val smokeDirectory = layout.buildDirectory.dir("conformance/m7-macos-smoke")
val smokeLog = "build/conformance/m7-macos-smoke/native-load.log"

val generateFfmBindings = tasks.register<JavaExec>("generateFfmBindings") {
    group = "build"
    description = "Generates production Objective-C runtime FFM bindings."
...
val generateNativeImageMetadata = tasks.register<JavaExec>("generateNativeImageMetadata") {
    group = "build"
    description = "Generates Native Image metadata for macOS bindings."
    classpath = ffiSchemaGenerator
...
pureJavaGuardConfig {
    ...
    nativeLoadTrace.set(layout.buildDirectory.file("conformance/m7-macos-smoke/native-load.log"))
}
...
val macosSmoke = tasks.register<JavaExec>("macosSmoke") {
    group = "verification"
